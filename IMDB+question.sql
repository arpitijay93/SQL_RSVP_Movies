...
select m.production_company , count(m.id) as movie_count , 
      dense_rank() over(order by count(m.id) desc) prod_company_rank
from ratings r
...
(select
    ...
    m.date_published,
    r.avg_rating,
    LEAD(m.date_published) OVER (partition by n.name ORDER BY date_published) next_movie_date,
    datediff(LEAD(m.date_published) OVER (partition by n.name ORDER BY date_published),m.date_published) as diff
from
	movie m, ratings r,
    names n, director_mapping dm
where
	m.id=r.movie_id
and n.id=dm.name_id
...
select 
    ...
    round(i.diff) avg_inter_movie_days,
...
from
	movie m, ratings r,
    names n, director_mapping dm,
	(select director_id, avg(diff) as diff from inter_movie group by director_id) i
where
	m.id=r.movie_id
and n.id=dm.name_id
and dm.movie_id=m.id
and i.director_id=n.id
group by
	n.name
